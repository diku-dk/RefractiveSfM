...
% Note: This code works only for planar calibration grid (checkerboard)
...
% using only the left most plane
Pall = Pall(:,1:48);
CameraRays = CameraRays(:,1:48);

% using the middle plane
% Pall = Pall(:,49:96);
% CameraRays = CameraRays(:,49:96);

% using the right plane
% Pall = Pall(:,97:end);
...
disp('==========8 pt =======================')
solVec = AxisEstimationPlanarScene(ray2D,Pall,mu);
ne8pt = solVec(1:3);
tau8pt = solVec(4);
