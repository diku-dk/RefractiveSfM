...
opt_z = [rand(1,2)*2-1,-1]';opt_z = opt_z ./ norm(opt_z);
rotaxis = cross([0;0;1],opt_z);
rotaxis = rotaxis ./ norm(rotaxis);
rotangle = acos(dot(opt_z,[0;0;1]));
% R = expSO3(theta*rotaxis);
% rotaxis = randn(3,1);%cross(n,[0,0,-1]');
% rotaxis = rotaxis./norm(rotaxis);
% rotangle = rand(1)*0.001;%acos(dot(n,[0,0,-1]'));
if abs(rotangle) > 1e-6
    Rgt = expSO3(rotangle.*rotaxis)';
    XYZnew = Rgt' * XYZ;
    Tgt = -Rgt*(d0 + 2 + rand(3,1).*5);%[0;0;-mean(XYZnew(3,:))];
    XYZ = XYZnew+(-Rgt'*Tgt);
end
w = Rot2RPY(Rgt);
...
disp('==========8 pt =======================')
solVec = EightPointEstimationOneLayerCase1KnownMu(ray2D,Pall,mu);
ne8pt = solVec(1:3);
tau8pt = solVec(4);
